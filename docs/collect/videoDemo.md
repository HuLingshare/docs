...
[DPlayer文档](https://dplayer.diygod.dev/zh/guide.html#special-thanks)

Demo来源：
[利用tensorflowjs实现B站的弹幕不遮挡人物效果](https://juejin.cn/post/7157955440549036069)
...
<html lang="en">
  <head>
    ...
    <link href="https://cdn.bootcdn.net/ajax/libs/vant/2.12.25/index.min.css" rel="stylesheet" type="text/css">
    ...
    <script src="https://unpkg.com/@mediapipe/selfie_segmentation"></script>
    <script src="https://unpkg.com/@tensorflow-models/body-segmentation"></script>
            ...
            <van-cell-group inset>
              ...
              <van-field label="模型分类">
                <template #label>
                  ...
                  <van-icon
                    name="question-o"
                    @click="showDialog('尺寸越大，识别越准确，同时性能也更差')"
                    style="padding: 0 3px"
                  />
                </template>
                <template #input>
              ...
              <van-field>
                <template #label>
                  ...
                  <van-icon
                    name="question-o"
                    @click="showDialog('前景阈值概率(越小物体越清晰，同时噪点也比较多)')"
                    style="padding: 0 3px"
                  />
                </template>
                <template #input>
  ...
  new Vue({
    ...
    methods: {
      ...
      async playViedo(item) {
        ...
        this.dp
          ? (this.dp.pause(), this.dp.danmaku.clear())
          : (toast.clear(), (this.loading = true))
      ...
      randomDanmaku() {
        const danmaku = this.actionViedo.barrage || [
      ...
      setVideoUrl(blobFile) {
        const url = window.URL && window.URL.createObjectURL(blobFile)
        window.URL && window.URL.revokeObjectURL(this.videoUrl)
        this.videoUrl = url
      },
      // 视频初始化
      dpInit() {
      ...
      async bodySegmentationInit() {
        try {
          ...
          const segmenterConfig = {
            runtime: 'mediapipe',
            modelType: this.modelType, //general  landscape
      ...
      async recognition() {
        const danmaku =
          this.$refs.dplayer &&
          this.$refs.dplayer.querySelector('.dplayer-danmaku')
        try {
          this.randomDanmaku()
          if (this.segmenter && this.maskOpen && danmaku) {
            ...
            const people = await this.segmenter.segmentPeople(
              imageData,
              segmentationConfig
            )
            ...
            const foregroundThresholdProbability =
              this.foregroundThresholdProbability //将像素着色为前景而不是背景的最小概率。
